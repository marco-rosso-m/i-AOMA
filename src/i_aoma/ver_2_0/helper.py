...
def run_SSICov_with_timeout(
    ...
    def run(
        ...
        output_path_phase1_stab_diag_backup,
    ):
        return run_SSICov(
            SingleSetup,
            qmc_sample,
    ...
    while getting_exception:
        try:
            ...
            if (
                qmc_limits["NDOFS"] * qmc_sample[0][0] < qmc_sample[0][1]
            ):  # raise Exception("Invalid QMC sample: NDOFS*br < ordmax !")
                discarded_qmc_samples.append([qmc_sample[0], qmc_sample_unitary[0]])
                print(
                    f"Analysis {NumAnal} interrupted: invalid QMC sample, i.e. NDOFS*br < ordmax !"
                )
            else:
...
def run_SSICov(
    ...
    if plt_stab_diag_backup:
        ...
        plt.savefig(
            f"{output_path_phase1_stab_diag_backup}/{NumAnal+1000}_StabDiag.png",
            dpi=200,
        )
        plt.close()
        ...
        plt.savefig(
            f"{output_path_phase1_stab_diag_backup}/{NumAnal+1000}_DampingClusters.png",
            dpi=200,
        )
        plt.close()
