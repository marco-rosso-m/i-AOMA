    ...
    for k in range(nSv):
        if k == 0:
            ax.plot(
                freq,
                10
                * np.log10(S_val[k, k, :] / S_val[k, k, :][np.argmax(S_val[k, k, :])]),
                "k",
                linewidth=2,
            )
        else:
            ax.plot(
                freq,
                10
                * np.log10(S_val[k, k, :] / S_val[0, 0, :][np.argmax(S_val[0, 0, :])]),
                "grey",
            )
...
def EFDD_FIT_plot(
    ...
    for numb_mode in range(len(PerPlot)):
        ...
        ax3.scatter(
            time[minmax_fit_idx], normSDOFcorr[minmax_fit_idx], c="r", marker="x"
        )
...
def set_view(ax: plt.Axes, view: str) -> plt.Axes:
    ...
    if view == "3D":
        ...
        raise ValueError(
            f"view must be one of (3D, xy, xz, yz), your input was '{view}'"
        )
    return ax
...
def plt_ch_info(
    ...
    for ii in range(nch):
        ...
        ylim = max(
            abs(np.sort(np.random.randn(n)).min()),
            abs(np.sort(np.random.randn(n)).max()),
        )
        ...
        sxn = np.sort(xn)
        ax21.plot(sorted_x, sxn, "k+", markersize=5, alpha=0.1)
        ax21.set_title(
        ...
        ax21.set_ylim(np.min(sxn), np.max(sxn))
