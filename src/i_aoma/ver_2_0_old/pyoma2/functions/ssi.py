...
def build_hank(
    ...
    if method == "cov_mm":
        ...
        with open("src/i_aoma/ver_2_0/Hank.npy", "rb") as f:
            Hank = np.load(f)
        ...
        if calc_unc is True:
            ...
            Hcov = np.zeros(
                ((p + 1) * l, q * r)
            )  # Averaged version of the Hankel matrix
    ...
    for ii in trange(1, ordmax + 1, step):
        ...
        if calc_unc is True:
            ...
            Q4_n = np.dot(  # noqa F841 (variable not used)
                np.hstack(
                    [np.eye(Nch * ii), np.zeros((Nch * ii, Nch * (ordmax - ii)))]
                ),
                Q4,
            )
            # Step 2 Algo2
...
def SSI_multi_setup(
    ...
    for kk in trange(n_setup):
        ...
        ref_id = np.array(
            [np.arange(br) * (n_ref + n_mov[kk]) + j for j in range(n_ref)]
        )
...
def SSI_mpe(
    ...
    if order == "find_min":
        ...
        for i in range(aggregated_poles.shape[1]):
            ...
            unique_poles = np.unique(
                current_order_poles[~np.isnan(current_order_poles)]
            )
